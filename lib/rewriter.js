(function() {
  var BALANCED_PAIRS, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_BLOCK, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, SINGLE_CLOSERS, SINGLE_LINERS, left, printLine, rite, _i, _len, _ref;
  ...
  exports.Rewriter = (function() {
    ...
    Rewriter.prototype.processComments = function() {
      return this.scanTokens(function(token, i, tokens) {
        ...
        printLine("Whoa!HERECOMMENT:" + token[1]);
                if ((_ref = (_base = this.tokens[i + 1]).comment) != null) {
          _ref;
        } else {
          _base.comment = '';
        };
        this.tokens[i + 1].comment += ((_ref2 = token.comment) != null ? _ref2 : '') + token[1];
        this.tokens.splice(i, 1);
        printLine("Spliced " + i + ":" + this.tokens[i]);
        return 0;
      });
    };
    ...
    Rewriter.prototype.removeMidExpressionNewlines = function() {
      return this.scanTokens(function(token, i, tokens) {
        var _ref, _ref2;
        if (!(token[0] === 'TERMINATOR' && ((_ref = this.tag(i + 1), __indexOf.call(EXPRESSION_CLOSE, _ref) >= 0) || (this.tag(i + 1) === 'HERECOMMENT' && this.tag(i + 2) === 'TERMINATOR' && (_ref2 = this.tag(i + 3), __indexOf.call(EXPRESSION_CLOSE, _ref2) >= 0))))) {
          return 1;
        }
        tokens.splice(i, 1);
        return 0;
    ...
    Rewriter.prototype.addImplicitIndentation = function() {
      return this.scanTokens(function(token, i, tokens) {
        var action, condition, indent, j, lastTag, outdent, starter, tag, _ref, _ref2;
        tag = token[0];
        lastTag = this.tag(i - 1);
        for (j = 1; j < 50; j++) {
          if (this.tag(i - j) !== 'TERMINATOR' && this.tag(i - j) !== 'HERECOMMENT') {
            lastTag = this.tag(i - j);
            break;
          }
        }

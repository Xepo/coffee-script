(function() {
  var Parser, alt, alternatives, grammar, name, o, operators, printLine, token, tokens, unwrap;
  ...
  o = function(patternString, action, options) {
    var i, match, origaction, param, params;
    patternString = patternString.replace(/\s{2,}/g, ' ');
    params = (function() {
      var _len, _ref, _results;
      _ref = patternString.split(' ');
      _results = [];
    ...
    params = '[' + params.join(',') + ']';
    if (!action) {
      return [patternString, "$$ = yy.surround(" + params + ", yylineno, $1);", options];
    }
    origaction = "" + action;
    ...
    return [patternString, "$$ = yy.surround(" + params + ", yylineno, " + action + ");", options];
  };
  grammar = {
    Root: [
      o('', function() {
